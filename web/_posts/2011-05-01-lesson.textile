...
You can take any function of multiple arguments and curry it. Let's try with our earlier {% highlight scala %}adder{% endhighlight %}
...
Earlier, you saw that we defined a function that took an <code>Int</code> which is a type of Number. Functions can also be generic and work on any type. When that occurs, you'll see a {% highlight scala %}type parameter{% endhighlight %} introduced with the square bracket syntax:
